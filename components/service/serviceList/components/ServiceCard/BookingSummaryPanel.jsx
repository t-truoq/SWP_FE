...
const BookingSummaryPanel = ({ selectedServices, onRemoveService, onClearAllServices, isLoggedIn }) => {
  ...
  useEffect(() => {
    const navbar = document.querySelector("nav"); // Sửa thành "nav" để khớp với Navbar.jsx
    if (navbar) {
  ...
  // Hàm xử lý khi nhấn "Book Selected Services"
  const handleBookServices = () => {
    ...
    // Chuẩn bị query parameters để truyền selectedServices
    const queryParams = new URLSearchParams();
    selectedServices.forEach((service, index) => {
      queryParams.append(`service${index}`, JSON.stringify(service));
    });

    // Chuyển hướng sang trang MyBooking với dữ liệu dịch vụ
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            style={{ top: `${navbarHeight + 10}px`, marginLeft: "20px" }}
                    ...
                    <div className="p-4 md:p-6">
                      {selectedServices.length === 0 ? (
                        ...
                        <>
                                  ...
                                  >
                                    <div>
                                      ...
                                      <div className="flex items-center text-sm text-gray-500 mt-1">
                                        {service.duration && (
                                          <>
                                            <Clock className="w-3.5 h-3. mr-1" />
                          ...
                          {/* Book Now Button */}
                          <motion.button
                            onClick={handleBookServices} // Sử dụng hàm mới thay vì onBookServices
